...
namespace airlib
{
    ...
    public: //types
        ...
        struct RotorParamsSettings
        {
            ...
            */
            float C_T = -1.0f; // the thrust co-efficient
            float C_P = -1.0f; // the torque co-efficient
            float air_density = 1.225f; //  kg/m^3
            float max_rpm = -1.0f; // revolutions per minute
            ...
            void applyDefaults() {
                C_T = 0.11f; // the thrust co-efficient @ 6396.667 RPM, measured by UIUC.
        ...
        struct MultirotorPhysicsSettings
        {
            std::string preset = "";
            int rotor_count = -1;
            float linear_drag_coefficient = -1.0f;
            ...
            float friction = 0.5f;
            std::vector<float> arm_lengths;
            std::vector<float> arm_angles;
            std::vector<std::string> rotor_directions;
            float mass = -1.0f;
            float motor_assembly_weight = -1.0f;
            std::vector<float> body_box;
            float rotor_z = -1.0f;
            ...
            void applyPresetDefaults(const std::string& preset_name) {
                ...
                if (!preset_name.empty()) {
                    ...
                    */
                    ...
                    rotor_count = 4;
                    linear_drag_coefficient = 0.325f;
                    angular_drag_coefficient = linear_drag_coefficient;
                    arm_lengths = std::vector<float>(rotor_count, 0.2275f);
                    arm_angles = {45.0f, -135.0f, -45.0f, 135.0f}; // Rotation according to NED right hand rule (so the vectors are rotated clockwise).
                    rotor_directions = {"ccw", "ccw", "cw", "cw"};
                    //set up mass
                    ...
                    mass = 1.0f;
                    motor_assembly_weight = 0.055f; // weight for MT2212 motor for F450 frame
                    body_box = {0.18f, 0.11f, 0.04f}; //set up dimensions of core body box or abdomen (not including arms).
                    rotor_z = 0.025f; // z relative to center of gravity
                    rotor_params.applyDefaults();
                }
