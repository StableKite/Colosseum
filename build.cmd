...
if %buildMode% == "" (
    ...
    msbuild -maxcpucount:12 /p:Platform=x64 /p:Configuration=Debug /verbosity:detailed /fl /flp:verbosity=detailed;logfile=MSBuild_Debug.log AirSim.sln
    ECHO(   
    if ERRORLEVEL 1 goto :buildfailed

    CALL :printHeader, "Building AirSim.sln Configuration = Release"
    msbuild -maxcpucount:12 /p:Platform=x64 /p:Configuration=Release /verbosity:detailed /fl /flp:verbosity=detailed;logfile=MSBuild_Release.log AirSim.sln
    ECHO(   
    if ERRORLEVEL 1 goto :buildfailed

    CALL :printHeader, "Building AirSim.sln Configuration = RelWithDebInfo"
    msbuild -maxcpucount:12 /p:Platform=x64 /p:Configuration=RelWithDebInfo /verbosity:detailed /fl /flp:verbosity=detailed;logfile=RelWithDebInfo_Release.log AirSim.sln
    ECHO(   
    if ERRORLEVEL 1 goto :buildfailed
) else (
    CALL :printHeader, "Building AirSim.sln Configuration = %buildMode%"
    msbuild -maxcpucount:12 /p:Platform=x64 /p:Configuration=%buildMode% /verbosity:detailed /fl /flp:verbosity=detailed;logfile=%buildMode%_Release.log AirSim.sln
    ECHO(   
    if ERRORLEVEL 1 goto :buildfailed
)
...
REM //---------- update all environments ----------
CALL :printHeader, "Building UE Project"
FOR /D %%E IN (Unreal\Environments\*) DO (
    cd %%E
    dotnet new globaljson --sdk-version 6.0.400 --force
    dotnet restore
    msbuild Blocks.sln /p:Configuration="Development Editor" /p:Platform=Win64 /t:Rebuild
    %powershell% -command "& \"%ProgramFiles%\Epic Games\UE_5.3\Engine\Build\BatchFiles\RunUAT.bat\" -ScriptsForProject=\"%ROOT_DIR%Unreal\Environments\Blocks\Blocks.uproject\" BuildCookRun -project=\"%ROOT_DIR%Unreal\Environments\Blocks\Blocks.uproject\" -noP4 -platform=Win64 -clientconfig=Development -serverconfig=Development -build -cook -pak -stage -archive -archivedirectory=\"%ROOT_DIR%Builds\""
    cd ..\..\..
)
